...
export default function HighlightedRenogramChart({ totalData, shapValues }: HighlightedRenogramChartProps) {

    const maxShap = Math.max(...shapValues);
    const minShap = Math.min(...shapValues);
    const normalizedShap = shapValues.map(val => (val - minShap) / (maxShap - minShap));

    const getHeatmapColor = (normalizedValue: number) => d3.interpolateWarm(1 - normalizedValue);

    const segmentSize = 18; // Each SHAP value represents 18 frames
    const segmentStartFrames = shapValues.map((_, index) => index * segmentSize + 1); // Start of each segment

    const segmentLabelPositions = segmentStartFrames.map(frame => frame + segmentSize / 2);
